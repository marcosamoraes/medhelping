    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div>
                    <section>
                        <form method="post" action="{{ route('articles.update', $article->id) }}" class="mt-6 space-y-6 flex flex-wrap gap-3" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="w-full lg:w-[32%]">
                                ...
                                <x-select-input id="category1" name="category1" type="text" class="mt-1 block w-full" required autofocus autocomplete="category1">
                                    ...
                                    @foreach ($categories as $id => $category)
                                        <option value="{{ $id }}" {{ old('category1', isset($article->categories[0]) ? $article->categories[0]->id : null) === $id ? 'selected' : false }}>{{ $category }}</option>
                                    @endforeach
                                </x-select-input>
