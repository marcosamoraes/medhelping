                    ...
                    <section>
                        <form method="post" action="{{ route('shifts.update', $shift->id) }}" class="mt-6 space-y-6 flex flex-wrap gap-3">
                            ...
                            <div class="w-full lg:w-[32%]">
                                ...
                                <x-select-input id="care_unit" name="care_unit" type="text" class="mt-1 block w-full" required autofocus autocomplete="care_unit">
                                    ...
                                    @foreach ($careUnits as $id => $careUnit)
                                        <option value="{{ $id }}" {{ old('care_unit', $shift->care_unit) === $id ? 'selected' : false }}>{{ $careUnit }}</option>
                                    @endforeach
                                </x-select-input>
                            ...
                            <div class="w-full lg:w-[32%]">
                                ...
                                <x-text-input id="value" name="value" type="time" class="mt-1 block w-full" :value="old('value', $shift->value)" required autofocus autocomplete="value" />
                                <x-input-error class="mt-2" :messages="$errors->get('value')" />
                            </div>

                            <div class="w-full lg:w-[32%]">
                                <x-input-label for="payment_method" value="Método de pagamento" />
                                <x-text-input id="payment_method" name="payment_method" type="text" class="mt-1 block w-full" :value="old('payment_method', $shift->payment_method)" required autofocus autocomplete="payment_method" />
